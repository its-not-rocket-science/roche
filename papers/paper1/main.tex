...
state-space transition matrices.  Empirically, the two certificates achieve similar (low) certification
rates under scalar and random-diagonal references; on dense normal and
non-normal families the determinant certifies marginally more instances, while
the resolvent is superior on diagonal-structured matrices and is numerically
better conditioned at large $n$.  Reference selection is the single largest
lever for reducing false negatives: diagonal-shrunk references achieve
100\,\% certification where random-diagonal search achieves $\leq 36$\,\%.
\end{abstract}
...
For diagonal test cases the grid-verified certificate becomes reliable above a
threshold grid size $N$ that grows with proximity to the unit circle.
No false positives occur for either the grid-verified or sampled certificate.
...
``Min $N$ (rig)'': smallest $N$ at which the grid-verified certificate passes.)}
...
framework, and admits a simple interpretation for diagonal SSMs.  Empirically,
the two certificates achieve comparable certification rates under scalar and
random-diagonal references; the determinant certifies marginally more instances
on dense normal and non-normal families, while the resolvent outperforms on
diagonal-structured matrices.  The resolvent's theoretical advantage---avoiding
polynomial ill-conditioning at large $n$---becomes decisive as state dimension
grows beyond $n \approx 64$.
