...
stable AR processes naturally remain stable and the eig-shrunk
reference suffices.  The contour barrier provides the best task-loss
preservation (closest to the unregularised baseline) while still
improving the resolvent margin from $+0.61$--$+0.73$ to $+0.82$.
The Lyapunov penalty is most aggressive: it drives $\varrho$ to
$\approx 0.46$ (vs.\ $0.76$--$0.84$ unregularised), at the cost of a
$24\%$ task-loss increase on the long-memory AR(4) task ($3.10$ vs.\
$2.51 \times 10^{-3}$), where long-range modes near the unit circle
are penalised away.  Spectral penalty occupies a middle ground,
halving $\varrho$ with $\leq 6\%$ task-loss overhead.
...
post-hoc certification, with the contour barrier best preserving task
performance ($\leq 8\%$ overhead) and the Lyapunov penalty incurring up
to 24\% task-loss increase on long-memory tasks due to over-suppression
of near-boundary modes.
...
tasks tested, matching the unregularised baseline.  The contour barrier
provides the best task-loss--margin trade-off: $\leq 8\%$ task-loss
overhead vs.\ $\leq 24\%$ for Lyapunov on long-memory tasks.  The
advantage of the contour barrier over spectral penalty is its
certificate-geometry awareness: it does not penalise stable modes
beyond the margin threshold $\tau$, preserving the long-range dynamics
needed for hard AR tasks.
