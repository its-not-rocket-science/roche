...
with rank-1 perturbation, $n=6$).  On the non-normal and Jordan-near-boundary
families of Table~\ref{tab:refquality}, no diagonal-class reference
certifies; the DLR path improves margins (less negative) but does not
achieve certification.  A systematic experiment comparing DLR to diagonal
references across all four families from Table~\ref{tab:refquality} is
deferred to future work; DLR convergence is slower than the diagonal path
due to the non-convex stability penalty and the larger parameter space.
...
enforced via sigmoid parameterisation; exact mean $\varrho^*$ from experimental run).}
...
constrained\_schur$^\dagger$ & 100 & \TBD{run exp} & 0 & 100 \\
\bottomrule
\end{tabular}
\end{table}
...
\hfill
\includegraphics[width=0.48\linewidth]{../../results/p2exp3/rho_trajectory.pdf}
...
below 0.98.
Right: per-epoch spectral radius trajectory (seed 0) for three
representative methods.  Unregularised (blue) and Lyapunov (green) escape
the unit circle within the first few epochs at lr=0.1; constrained\_schur
(purple) cannot exceed 0.98 by construction.}
\label{fig:adversarial}
\end{figure}

\paragraph{Failure mechanism.}
Figure~\ref{fig:adversarial} (right) shows the per-epoch $\varrho$
trajectory for one representative seed.  At lr = 0.1, Adam momentum
can carry eigenvalue radii by $\Delta\varrho \approx 0.15$ per step.
