...
controlled margin and degree, the Rouch\'e certificate matches the simpler
real-polynomial positivity check---both achieve near-100\% certification at
large margins and degrade gracefully as margin shrinks.  On a small MLP
...
\begin{enumerate}
  ...
  \item Experiments on synthetic polynomial classifiers
    (Section~\ref{sec:exp_synthetic}) confirming the certificate works when
    surrogates are accurate.
...
path margins of order 0.1--1.  Rouch\'e matches real poly at degrees 4 and
8 (cert rates 66.5\% and 67.0\%) but collapses to 34.5\% at degree~16,
...
Based on the synthetic experiments (Section~\ref{sec:exp_synthetic}),
Rouch\'e and real-polynomial certificates are essentially equivalent in
certification rate: both succeed when the surrogate minimum is large relative
to the approximation error.  The Rouch\'e certificate adds computational cost
(complex contour evaluation) without a corresponding reduction in false
negatives.
...
Empirically, the Rouch\'e certificate matches the simpler real-polynomial
check on both synthetic and neural network settings.  On realistic neural
paths beyond short interpolation scales, both certificates become vacuous
because surrogate approximation error consumes the available margin.
