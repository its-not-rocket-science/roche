...
\begin{enumerate}
  \item A sound theorem (Theorem~\ref{thm:main}) connecting Rouch\'e
    ...
    with an explicit remark (Remark~\ref{rem:epsilon}) distinguishing the
    numerical $\epsilon_j$ used in experiments from a verified bound.
  \item An analysis (Section~\ref{sec:real_baseline}) explaining why the
    real-polynomial positivity check is tighter, cheaper, and free of the
    second failure mode that affects Rouch\'e.
  \item A quantitative account of complex excursion
    (Section~\ref{sec:excursion}): a degree-8 example where positivity on
    $[0,1]$ is trivial but contour values span $[\varepsilon, 72]$, making
    the Rouch\'e condition structurally impossible to satisfy.
  \item A counterexample (Section~\ref{sec:counterexample}) showing that naive
    complexification of network activations destroys analyticity (poles inside
    $D$), not merely introduces spurious zeros.
