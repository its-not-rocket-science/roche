...
def run_cell(degree: int, min_margin: float, rng: np.random.Generator) -> dict:
    counts = {"dense": 0, "real_poly": 0, "rouche": 0}
    margins = {"real_poly": [], "rouche": []}

    for _ in range(N_INSTANCES):
        g_fn = make_random_margin_fn(degree, min_margin, rng)
        results = certify_path(
            g_fn,
            degree=degree,
    ...
    return {
        "cert_rate": {m: counts[m] / N_INSTANCES * 100 for m in counts},
...
def main() -> None:
    ...
    for deg, mm in product(DEGREES, MIN_MARGINS):
        label = f"  deg={deg} min={mm:.2f}"
        print(label, flush=True)
        cell = run_cell(deg, mm, rng)
    ...
    fig.tight_layout()
    fig.savefig(RESULTS_DIR / "cert_rate_heatmap.pdf", bbox_inches="tight")
    plt.close(fig)
    ...
    ax2.legend()
    fig2.tight_layout()
    fig2.savefig(RESULTS_DIR / "margin_distribution.pdf", bbox_inches="tight")
    plt.close(fig2)
